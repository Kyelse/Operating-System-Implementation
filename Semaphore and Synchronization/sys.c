 ...
 * and  decrements the value (i.e., uses up one stored wakeup), and just 
 * continues. If the value is 0, the process is added into the queue of waiting 
 * for wake up put to sleep without completing the down for the moment. All of the 
 * operation is being done within one atomic action only.
 */
SYSCALL_DEFINE1(csc452_down, csc452_sem *, sem){
	mutex_lock(&sem_lock);
	// decrement the value of the semaphore 
	sem->value = sem->value - 1; 
	if (sem->value < 0) {
		// add the current process into the queue 
		process_queue *head, *newNode, *temp; 
		head = sem->process; 
		newNode = (process_queue*) kmalloc(sizeof(process_queue), GFP_ATOMIC);
		...
		// call Sleep() according to kernel convention 
 ...
 * more processes were sleeping on that semaphore (i.e. the value is still less than 0) 
 * a task from the process queue would being chosen to wake up. All of the 
 * operation is being done within one atomic action only.
 */
SYSCALL_DEFINE1(csc452_up, csc452_sem *, sem){
	mutex_lock(&sem_lock);
	sem->value = sem->value + 1;
	if (sem->value <= 0) {
		process_queue *head; 
		struct task_struct *task_wake; 
		// remove the longest waiting process out of the queue
		head = sem->process; 
		if (head != NULL) {
			task_wake = head->task; 
			sem->process = head->next; 
			wake_up_process(task_wake); 
		}
		kfree(head);
	}
	mutex_unlock(&sem_lock); 
